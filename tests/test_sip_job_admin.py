...
def test_run_sip_job_is_idempotent_per_day(db_session, monkeypatch):
    ...
    first = run_sip_job(db_session, trigger="MANUAL", force=False)
    second = run_sip_job(db_session, trigger="MANUAL", force=False)
...
def test_get_sip_job_status_includes_totals(db_session):
    ...
    status = get_sip_job_status(db_session)
...
def test_admin_endpoints_expose_status_and_manual_run(client, db_session, monkeypatch):
    db_session.add(
        ImportedHolding(
            symbol="AXISMF",
            company_name="Axis Flexicap",
    ...
    run_response = client.post("/admin/sips/run")
    assert run_response.status_code == 200
    assert run_response.json()["status"] in {"success", "skipped"}

    status_response = client.get("/admin/sips/status")
